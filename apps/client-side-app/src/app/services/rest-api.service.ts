...
export class RestApiService {
  ...
  getUnwatchedMovies(): Observable<Movie> {
    return this.http
      .get<Movie>(this.apiURL + '/unwatched')
      .pipe(retry(1), catchError(this.handleError));
  }
  ...
  getRecommendedMovie(id): Observable<Movie> {
    return this.http
      .get<Movie>(this.apiURL + '/recommendation/' + id)
      .pipe(retry(1), catchError(this.handleError));
  }

  //HttpClient API get() method => Get search results
  getSearchMovie(title): Observable<Movie> {
    return this.http
      .get<Movie>(this.apiURL + '/search/' + title)
      .pipe(retry(1), catchError(this.handleError));
  }
  ...
  postAddMovie(movie: Movie): Observable<Movie> {
    return this.http
      .post<Movie>(this.apiURL + '/add', movie, this.httpOptions)
      .pipe(retry(1), catchError(this.handleError));
  }
  ...
  patchWatchMovie(id): Observable<Movie> {
    return this.http
      .patch<Movie>(this.apiURL + '/watch/' + id, this.httpOptions)
      .pipe(retry(1), catchError(this.handleError));
  }
  ...
  deleteMovie(id): Observable<Movie> {
    return this.http
      .delete<Movie>(this.apiURL + '/delete/' + id)
      .pipe(retry(1), catchError(this.handleError));
  }
